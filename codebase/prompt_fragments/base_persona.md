...
- **Important:** The project source code may live inside a `codebase/` subdirectory under the workspace root. Paths from AGENTS.md or user tasks may be relative to `codebase/`, not the workspace root. If a path like `temp/dummy/fibo/fibonacci.py` fails with "not found" or "not a directory", try prefixing with `codebase/` before concluding it doesn't exist.
