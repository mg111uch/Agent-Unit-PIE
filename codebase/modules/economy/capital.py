...
def _sums(unit: Optional[str], db_path: Optional[str]) -> float:
    con = ledger.connect(db_path)
    try:
        if unit is None:
            ...
            inn = con.execute("SELECT COALESCE(SUM(amount),0) FROM transactions "
                              "WHERE recipient=? AND status='posted'", (unit,)).fetchone()[0]
            out = con.execute("SELECT COALESCE(SUM(amount),0) FROM transactions "
                              "WHERE sender=? AND status='posted'", (unit,)).fetchone()[0]
        return round(float(inn) - float(out), 2)
    finally:
        con.close()


def earn(amount: float, unit: str, desc: str = "",
         db_path: Optional[str] = None) -> str:
    """Book income: external -> unit. Returns tx_id."""
    _migrate(db_path)
    v = _paise(amount)
    if v <= 0:
        raise ValueError("earn amount must be > 0")
    return _insert(make_tx(EXT, unit, v, "income", _now()), "posted", db_path)


def spend(amount: float, unit: str, desc: str = "",
          db_path: Optional[str] = None) -> str:
    """Book expense: unit -> external. Returns tx_id."""
    _migrate(db_path)
    v = _paise(amount)
    if v <= 0:
        raise ValueError("spend amount must be > 0")
    return _insert(make_tx(unit, EXT, v, "expense", _now()), "posted", db_path)


def balance(unit_id: str, db_path: Optional[str] = None) -> float:
    """Cash balance: posted credits - debits (proposals excluded)."""
    _migrate(db_path)
    return _sums(unit_id, db_path)
...
def propose_action(desc: str, cost: float, unit: str = EXT,
    ...
    tid = _insert(make_tx(unit, EXT, v, "proposed", _now()), "proposed", db_path)
