"""
metadata:
  summary: "This Python file provides functions to run the agricultural simulation for a set number of years with progress logging, and to generate matplotlib visualizations of key metrics including population, wealth, skills, births/deaths, and specialist agent impacts."
  dependencies: ["model.py", "constants.py"]
  tags: ["simulation", "plotting", "matplotlib", "visualization", "analysis"]
  hierarchy_mapping:
    classes: {}
    functions: ["run_simulation", "plot_results"]
  graph_methods:
    dependency_graph: {"nodes": [{"id": "run_simulation", "type": "function"}, {"id": "plot_results", "type": "function"}, {"id": "AgriculturalModel", "type": "class"}], "edges": [{"from": "run_simulation", "to": "AgriculturalModel", "type": "instantiates"}, {"from": "run_simulation", "to": "AgriculturalModel", "type": "calls"}, {"from": "plot_results", "to": "pd.DataFrame", "type": "uses"}]}
    cfg_outline: "run_simulation: initialize model; for each year: step model, log metrics. plot_results: create subplots; plot population, wealth, skill, births/deaths bar, healer/toolmaker/trader counts, cumulative impacts."
"""
# === Imports ===
import numpy as np
import pandas as pd
