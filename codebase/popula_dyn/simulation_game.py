"""
metadata:
   summary: "This Python file implements a FastAPI backend for an interactive population simulation game with fertility-based movement support, offering REST API endpoints for simulation control and WebSocket support for real-time state updates and agent visualization."
   dependencies: ["model.py", "constants.py"]
   tags: ["fastapi", "backend", "websocket", "simulation", "game", "fertility_movement"]
   hierarchy_mapping:
     classes: {}
     functions: ["start_simulation", "stop_simulation", "reset_simulation", "update_params", "get_simulation_state", "step_simulation", "simulation_websocket", "broadcast_simulation_updates", "startup_event"]
   graph_methods:
     dependency_graph: {"nodes": [{"id": "AgriculturalModel", "type": "class"}, {"id": "start_simulation", "type": "function"}, {"id": "get_simulation_state", "type": "function"}, {"id": "step_simulation", "type": "function"}, {"id": "simulation_websocket", "type": "function"}, {"id": "broadcast_simulation_updates", "type": "function"}], "edges": [{"from": "start_simulation", "to": "AgriculturalModel", "type": "instantiates"}, {"from": "get_simulation_state", "to": "AgriculturalModel", "type": "queries"}, {"from": "step_simulation", "to": "AgriculturalModel", "type": "calls"}, {"from": "broadcast_simulation_updates", "to": "step_simulation", "type": "calls"}, {"from": "broadcast_simulation_updates", "to": "get_simulation_state", "type": "calls"}]}
     cfg_outline: "On startup, create broadcast task. WebSocket accepts connections, handles messages (start/stop/reset/step/update_params including fertility_movement), sends state. Background task: if running, step simulation, get state, broadcast to clients every second."
"""
# === FastAPI Backend for Simulation Game ===
