...
def register_questions(session_id: str, questions: list) -> Optional[str]:
    """Validate and register pending questions WITHOUT blocking.

    Returns an error string on failure, or None when registered.
    Call wait_for_questions(session_id) to block until the user answers.
    """
    if not session_id:
...
def wait_for_questions(session_id: str) -> str:
    """Block until the user answers an already-registered question set."""
    ...
    result = _pending[session_id].get("answers")
...
def ask_user_question(raw_input: Any) -> str:
    ...
    error = register_questions(_session_id, questions)
    if error:
        return error
    return wait_for_questions(_session_id)
