...
_FILE_SPECS = [
    ...
    ("Write", Write, CAT_FILE,
     ...
     {"path": str_p("file path rel workspace", req=True),
      ...
      "content": str_p("file content"),
      "dry_run": bool_p("validate, no write")}),
    ("edit_file", edit_file, CAT_FILE,
     "Replace exact old_string→new_string in existing file. old_string must match once (or replace_all=true). edits=[...] sequential, checkpoint+cache parity.",
     {"path": str_p("file path rel workspace", req=True),
      "old_string": str_p("exact text to replace, whitespace-sensitive", req=True),
      "new_string": str_p("replacement text", req=True),
      "replace_all": bool_p("replace all old_string runs (default false)"),
      "edits": {"t": "array", "desc": "sequential edits (vs single old/new)", "r": False,
                "items": {"type": "object", "properties": {
                    "old_string": {"type": "string", "description": "exact text to replace"},
                    "new_string": {"type": "string", "description": "replacement"},
                    "replace_all": {"type": "boolean", "description": "all runs (default first)"}},
    ...
    ("grep_search", grep_search, CAT_FILE,
     ...
     {"pattern": str_p("regex for file contents", req=True),
      "include": str_p("file glob filter, e.g. '*.py' or '*.{py,ts}'"),
      "context_lines": int_p("context lines before/after match (default 0)"),
      "max_results": int_p("max result lines (default 50)")}),
    ("todo", todo, CAT_FILE,
     "Task plan. action: read/create/update/mark_done/clear.",
     {"action": str_p("read|create|update|mark_done|clear", req=True),
      "items": arr_p("string", "task descriptions (create/update)"),
      "ids": arr_p("integer", "task ids to mark done")}),
("ask_user_question", ask_user_question, CAT_FILE,
      "Ask user input/clarification/decision for ALL interactive questions in chat UI—never type them in reply. Up to 3 options per question (custom-answer box always available); multiple questions answered one by one.",
      {"questions": {"t": "array", "desc": "questions, answered sequentially. max 3 options each.", "r": True,
                     "items": {"type": "object", "properties": {
                         "question": {"type": "string", "description": "question text"},
                         "header": {"type": "string", "description": "short title"},
                         "options": {"type": "array",
                                     "items": {"anyOf": [
                                         {"type": "string"},
                                         {"type": "object", "properties": {
                                             "label": {"type": "string", "description": "answer label"},
                                             "description": {"type": "string", "description": "hint under label"}},
