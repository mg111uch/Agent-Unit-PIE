...
- 2026-09-15: Economic event adapter shipped — FireFlow-shaped `PaymentSettled` → kernel event + `capital_flow` signal via `kernel_bus` (`event_adapter.py`, proxy pattern, no external calls); smoke ok on tmp DB.
